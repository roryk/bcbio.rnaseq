...
library(HTSFilter)
library(tools)
count_file = "de-analysis/combined.counts"
out_file = "de-analysis/deseq_A_vs_B.tsv"
class = factor(c("A","B","A","B","A","B","A","B","A","B"), levels=c("A","B"))
...
out_table = data.frame(id=res$id, expr=res$baseMean, logFC=res$log2FoldChange,
          pval=res$pval, padj=res$padj, algorithm="deseq")
write.table(out_table, file=out_file, quote=FALSE, row.names=FALSE,
            sep="\t")
write.table(counts(DESeq.cds, normalized=TRUE), file=normalized_file, quote=FALSE, sep="\t")
